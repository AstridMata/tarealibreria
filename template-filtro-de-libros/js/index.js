...
function filtrarAutor (arr,autor){
    const filtrados = arr.filter ((item) => item.autor === autor);
    return filtrados; 
}

function mostrarFiltrados(){
    let inputFiltro = document.getElementById("autor");
    let divLibros = document.getElementById("libros");

//llamar a filtrar auto y tomar el resultado
const arr = filtrarAutor (libros, inputFiltro.value);
